...
```properties-vue [1.20.1]
ponder_version = {{ ciData.get("1.20.1") }}
```
...
```groovy [build.gradle] {3,6,9,12,15}
dependencies {
    // Fabric
    modImplementation(include("net.createmod.ponder:ponder-fabric:${ponder_version}+mc${minecraft_version}"))

    // Forge (ForgeGradle)
    implementation(jarJar(fg.deobf("net.createmod.ponder:ponder-forge:${ponder_version}+mc${minecraft_version}")))

    // Forge (ModDevGradle Legacy)
    modImplementation(jarJar("net.createmod.ponder:ponder-forge:${ponder_version}+mc${minecraft_version}"))
...
```kotlin [build.gradle.kts] {3,6,9,12,15}
dependencies {
    // Fabric
    modImplementation(include("net.createmod.ponder:ponder-fabric:${ponder_version}+mc${minecraft_version}")!!)
